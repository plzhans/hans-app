...
#
# 런타임 동작 기준일(compatibility_date)은 이 프로젝트의 wrangler.jsonc 가 정한다 —
# 여기서 밀어 넣지 않는다. 그 날짜는 "어느 런타임 동작에 고정할 것인가" 라서, 올리는 사람과
# 검증하는 사람이 같아야 한다.
set -euo pipefail
...
#
# `wrangler deploy --name X` 는 X 가 없으면 만들고 있으면 덮어쓴다. 편해 보이지만 이름을
# 잘못 준 배포가 조용히 "성공" 한다 — 엉뚱한 Worker 가 새로 생기고 정작 보고 있는 사이트는
# 안 바뀐다. **"없음" 과 "권한 없음" 을 구분한다** — 뭉뚱그리면 토큰 권한이 모자란 상황에서
# "처음 만드는 거니 CF_ALLOW_CREATE 를 주라" 고 엉뚱하게 안내하게 된다.
...
case "$http_code" in
  ...
  404)
    # 첫 생성은 의사표시를 받는다. CI 는 물어볼 상대가 없으므로 CF_ALLOW_CREATE 를 요구한다 —
    # 조용히 만들면 오타 하나가 유령 Worker 를 남긴다.
...
# **워커 스크립트가 읽는 값은 --var 로 넘긴다.** wrangler.jsonc 에 박지 않는 이유는
# 환경마다 달라지기 때문이고, 별명을 만들지 않는 이유는 매핑표가 또 하나의 틀릴 수 있는
# 자리가 되기 때문이다. 이름은 .env 의 것을 그대로 쓴다.
...
# dlx 는 임시 스토어를 쓰므로 프로젝트의 allowBuilds 설정이 닿지 않는다.
