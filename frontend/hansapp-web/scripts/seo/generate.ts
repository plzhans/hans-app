 ...
 *
 * 약관·방침은 **크롤러를 들여보내되 색인만 막는다.** robots.txt 로 막으면 크롤러가 문 앞에서
 * 돌아가 이 noindex 를 못 읽고, 스토어·심사 양식에 적어 둔 주소가 내용 없이 URL 만 검색
 * 결과에 남는다. follow 는 남긴다 — 이 문서를 색인하지 말라는 것이지 링크까지 끊을 이유는 없다.
 *
