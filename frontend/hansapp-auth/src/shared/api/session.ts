 ...
 *
 * **이게 있을 때만** refresh 를 호출한다. refresh 쿠키 자체는 httpOnly 라 JS 가 못 읽어서,
 * 그것만으로는 "로그인했나"를 알려면 매번 서버를 때려야 한다 — 로그아웃 상태에서도.
 * 그래서 같은 수명·도메인으로 읽을 수 있는 flag 를 하나 같이 심는다(백엔드 setSessionHint).
 *
 * 인증 판단이 아니라 **"호출 여부" 판단용**이다. 값은 신뢰하지 않는다 — 사용자가 직접
 * 만들어 넣을 수 있으니, 있으면 refresh 를 시도해보고 서버 응답으로 판정한다.
 */
