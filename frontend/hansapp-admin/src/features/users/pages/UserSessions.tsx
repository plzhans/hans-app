...
import {
  getUser,
  getUserSessionCacheState,
  listUserSessions,
  purgeUserSessionCache,
  revokeAllUserSessions,
  revokeUserSession,
  type OrphanSessionCache,
  type UserSession,
...
/*
  첫 칸은 세션 식별자다. **24자 고정**(랜덤 18바이트의 base64url)이라 폭을 그에 맞춰 두면
  잘리지 않는다. 마지막 칸은 버튼 하나가 들어갈 자리인데, 글자 폭보다 넉넉히 준다 —
  딱 맞춰 두면 글꼴이 조금만 달라져도 버튼이 잘린다.
*/
const COLUMNS =
  'grid-cols-[190px_minmax(0,1fr)_140px_120px_120px_120px_150px_104px] gap-4 px-6';
...
export default function UserSessions() {
  ...
  /** JSON 을 펼쳐 보고 있는 세션. 닫혀 있으면 null. */
 ...
 * 기기가 아직 통한다**는 뜻이다.
 *
 * 정리 배치가 주기적으로 치우지만, 그때까지 열려 있는 것이 문제라 여기서 바로 지운다.
 */
  ...
  return (
    <section className="mt-6 rounded-2xl border border-amber-300 bg-amber-50 p-6">
      ...
      <p className="mt-1 text-sm text-amber-800">
        세션은 지워졌는데 캐시가 남아 있습니다. 그 기기는 캐시가 만료될 때까지 계속
        통과합니다 — 끊을 때 캐시 삭제가 실패하면 생깁니다. 정리 배치가 나중에
        치우지만, 여기서 바로 지우는 편이 낫습니다.
      </p>
      ...
      <ul className="mt-4 space-y-2">
        {orphans.map((orphan) => (
          <OrphanRow
            ...
            orphan={orphan}
            onViewCache={() => onViewCache(orphan.sessionId)}
          />
        ))}
      </ul>
    </section>
...
function OrphanRow({
  ...
  orphan: OrphanSessionCache;
  onViewCache: () => void;
}) {
  return (
    <li className="flex flex-wrap items-center gap-3 rounded-xl border border-amber-200 bg-white px-4 py-3">
...
function CacheJsonModal({
  ...
  const query = useQuery({
    queryKey: ['user-session-cache', userId, sessionId],
    queryFn: () => getUserSessionCacheState(userId, sessionId),
  });
  const state = query.data;

  const purge = useMutation({
    mutationFn: () => purgeUserSessionCache(userId, sessionId),
    onSuccess: async () => {
      /*
      ...
      */
      await qc.invalidateQueries({ queryKey: ['user-sessions', userId] });
      onClose();
    },
  });
          ...
          <>
            ...
            */}
            ...
            <p className="mt-2 text-xs text-gray-400">
              바깥의 <span className="font-mono">v</span> 는 캐시 미스와 "조회했는데
              없음" 을 가르려고 서버가 씌운 껍데기입니다.
            </p>
          </>
        ) : (
