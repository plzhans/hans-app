 ...
 * 옮겨 적어야 하는 값.
 *
 * **복사 버튼이 실패할 수 있다.** `navigator.clipboard` 는 보안 컨텍스트(https·localhost)
 * 에서만 있고, 브라우저에 따라 권한이나 사용자 제스처를 더 따진다. 그래서
 *
 *   1. 되면 그걸 쓰고,
 *   2. 안 되면 옛 방식(execCommand)으로 한 번 더 시도하고,
 *   3. 그것도 안 되면 **실패했다고 말한다** — 조용히 삼키면 복사한 줄 알고 창을 닫는다.
 *
