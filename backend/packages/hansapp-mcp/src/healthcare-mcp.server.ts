 ...
 *
 * [모듈이 아니라 클래스만 내보내는 이유]
 * 도구가 부르는 서비스(HealthcareHospitalService 등)는 ApplicationModule 이 소유하는데,
 * 여기서 모듈로 감싸면 그걸 쓰려고 ApplicationModule.forRoot 를 또 불러야 한다 — 동적 모듈이라
 * 두 번 부르면 인스턴스가 두 벌이 되고 Prisma 풀·코드 캐시가 같이 두 벌 뜬다.
 * 그래서 이 패키지는 **도구 정의만** 소유하고, 배선은 서버 앱이 프로바이더로 등록해 끝낸다.
 *
 * [도구를 셋으로 묶은 이유]
