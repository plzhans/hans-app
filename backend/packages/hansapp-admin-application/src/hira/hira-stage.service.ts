...
/**
 * 단계별 한 줄 설명. 관리자 화면이 "무엇을 끄는지" 를 이걸로 보여준다.
 *
 * **콜 수를 적어 둔다.** 단계를 끄는 이유가 거의 항상 한도라서, 끄는 사람이 판단할 값은
 * 이름이 아니라 무게다. 1단계만 목록이고 나머지는 전부 병원 하나당 10콜짜리 개별 상세다.
 */
export const HIRA_STAGE_DESCRIPTIONS: Record<HiraStage, string> = {
  1: '목록 — 코드표 6종 + 전체 병원 + 진료과목·전문병원 역조회 + 평가등급 + 비급여 (약 280콜)',
  2: '개별 상세 — 상급종합 47개 × 10종',
  3: '개별 상세 — 종합병원 338개 × 10종',
  4: '개별 상세 — 병원 1,429개 × 10종',
  5: '개별 상세 — 정신병원 261개 × 10종',
  6: '개별 상세 — 한방병원 624개 × 10종',
  7: '개별 상세 — 치과병원 247개 × 10종',
  8: '개별 상세 — 요양병원 1,285개 × 10종',
  9: '개별 상세 — 의원 37,789개 × 10종 (운영계정 필요)',
  10: '개별 상세 — 치과의원 19,380개 × 10종 (운영계정 필요)',
  11: '개별 상세 — 한의원 14,873개 × 10종 (운영계정 필요)',
  12: '개별 상세 — 보건기관 등 나머지 × 10종 (운영계정 필요)',
};
