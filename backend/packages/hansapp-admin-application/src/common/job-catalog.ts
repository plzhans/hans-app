...
/**
 * 배치가 돌리는 잡의 목록. **이름·설명·분류·기본 주기의 정본이다.**
 *
 * 스케줄러 등록명·Sentry 감시 이름·batch_job 의 키가 전부 여기 한 값을 본다 — 여러 곳에
 * 문자열을 따로 박으면 한쪽만 바뀌어 조용히 어긋난다.
 *
 * **배치 앱이 아니라 여기 있다.** 돌리는 것은 배치지만 이 목록을 보는 쪽은 둘이다 —
 * 배치가 부팅 때 크론을 걸고, 관리자 API 가 문서와 검증에 쓴다(단계 목록 stageCatalog 과
 * 같은 사정이다). 배치 안에 두면 관리자 쪽이 잡 이름을 따로 적게 되고, 그 사본은 반드시 낡는다.
 *
 * ## 잡을 무엇 단위로 자르나
 *
