...
/**
 * NMC 병원 미러 저장소. 적재(mirror upsert)와 지역 집계 재생성만 담당한다.
 *
 ...
 */
@Injectable()
export class NmcHospitalSyncRepository {
  constructor(private readonly prisma: PrismaService) {}

  /** nmc_hospital 에 hpid 키로 벌크 upsert. 처리한 행 수를 반환한다. */
  upsertMirror(rows: MirrorRow[]): Promise<number> {
    return upsertMirrorRows(this.prisma, 'nmc_hospital', 'hpid', rows);
  }

  /** 병원 데이터에서 지역 집계(nmc_region)를 다시 만든다. */
  rebuildRegions() {
    return rebuildNmcRegions(this.prisma);
  }
}
