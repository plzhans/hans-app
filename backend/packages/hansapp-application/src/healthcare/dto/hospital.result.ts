...
export interface HospitalSummary {
  ...
  /**
   * 이 병원 정보가 마지막으로 바뀐 시각(ISO 8601).
   *
   * 원본(HIRA·NMC)은 수정 시각을 주지 않는다. 빌드가 회차마다 만든 값을 직전 회차와 맞대
   * 보고 달라진 병원에만 찍으므로, **원본이 바뀐 시각이 아니라 우리가 변화를 본 시각**이고
   * 회차 주기만큼 늦다. 진료과목·진료시간 같은 자식 정보가 바뀐 것도 여기에 반영된다.
   */
  updatedAt: string;
