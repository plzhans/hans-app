...
export interface HealthcareHospitalDoc {
  ...
  /**
   * 원문 이름(법인 표기를 떼기 전). **name 과 다를 때만 담는다.**
   *
   * "고려중앙학원" 으로 찾는 사람이 있고, 무엇보다 분리 규칙이 법인명을 못 가른 병원은
   * 원문으로만 찾힌다. name 과 같은 값을 또 넣으면 tf 가 부풀어 그 병원만 점수가
   * 올라가는데, 98.7% 가 그렇게 되면 랭킹이 통째로 흔들린다.
   *
   * 한국어만이다 — 번역 대상은 짧은 이름(name)이고 원문은 번역하지 않는다.
   */
  legal_name?: string;

  /** 법인격 + 법인명("의료법인 일맥의료재단"). 재단·계열로 묶어 찾을 때 쓴다. */
  corp_name?: string;
  ...
  location: {
     ...
     * 시도 정렬 순위. **작을수록 앞이다.**
     *
     * 조건 없이 열었을 때 무엇이 먼저 보이는가를 정한다 — 예전에는 전건이 동점(_score 0)이라
     * 색인 순서(id)로 나왔고, 첫 20 건이 사실상 무작위였다.
     *
     * **인구 순이 아니라 방문자 기준이다.** 인구만 보면 경기가 서울보다 많지만, 이 서비스는
     * 외국인 방문자를 염두에 두고 있어 그들이 머무는 곳이 먼저여야 한다. 게다가 경기는 넓어서
     * "경기도 병원" 이 서울에서 한 시간 넘게 걸리는 경우가 흔하다.
     *
     * **점수(_score)가 아니라 별도 필드다.** 점수에 가점으로 얹으면 키워드 관련도와 섞여
     * 왜 이 순서인지 설명할 수 없게 되고, search_after 커서가 점수에 걸려 있어 가중치를
     * 손볼 때마다 스크롤 중이던 사용자가 어긋난다. 필드로 두면 정렬 배열에 우선순위가
     * 그대로 드러난다: [_score, sido_rank, id].
     */
    sido_rank?: number;
    region_cd?: string;
    emdong_nm?: { ko?: string };
