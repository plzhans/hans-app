...
import { ElasticsearchService, SearchSchemaService } from '@hansapp/search';
import { HealthcareIndexService } from '@hansapp/admin-application';
...
function registerHospital(es: Command, source: ConfigSource): void {
  ...
  addExamples(
    ...
    [
      'hansapp-cli es hospital sync',
      'hansapp-cli es hospital sync --prune',
      'hansapp-cli es hospital sync --fresh',
      'hansapp-cli es hospital sync --quiet',
    ],
  );
