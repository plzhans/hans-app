...
function addNpayWebCommands(hira: Command, source: ConfigSource): void {
  ...
  addExamples(
    ...
    [
      'hansapp-cli hira npay-web work            # 큐에서 1건 처리',
      'hansapp-cli hira npay-web work -n 10      # 10건 처리',
    ],
  );
...
export function hiraCommand(source: ConfigSource): Command {
  ...
  addExamples(
    ...
    [
      'hansapp-cli hira hospital list --simple                        # 로컬 DB',
      'hansapp-cli hira hospital list --origin --sido 110000 --simple # 원본 API + 필터',
    ],
  );
  ...
  addExamples(
    ...
    [
      'hansapp-cli hira code list --tp class --simple',
      'hansapp-cli hira code list --tp subject --origin --simple',
    ],
  );
