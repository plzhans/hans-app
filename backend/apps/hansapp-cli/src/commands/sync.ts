...
export function syncCommand(
  ...
  return addExamples(command, [
    `hansapp-cli ${provider} hospital sync            # 1건만 (기본값)`,
    `hansapp-cli ${provider} hospital sync -n 100     # 1페이지 100건`,
    `hansapp-cli ${provider} hospital sync -p 3 -n 100  # 3페이지만`,
    `hansapp-cli ${provider} hospital sync --full     # 전체 (${DEFAULT_SYNC_ROWS}건씩)`,
  ]);
}
