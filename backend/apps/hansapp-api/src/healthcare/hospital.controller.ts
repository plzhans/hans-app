...
export class HealthcareHospitalController {
  ...
  @ApiOperation({
    ...
    description:
      ...
      '겹친 과목은 **matchedSubjects** 로 함께 반환하므로, 결과에 포함된 근거를 화면에 ' +
      '표시할 수 있다.\n\n' +
  ...
  @ApiOperation({
    ...
    description:
      ...
      '**빈 categories 도 정상이다.** 비급여를 신고한 기관은 전체의 약 4%(3,511곳)이며, ' +
      '의원급(clCd=31)은 원본에 없어 항상 비어 있다. 404 는 병원 자체가 없을 때만 반환한다.\n\n' +
      '**금액은 범위다.** 한 표준코드에 원본 행이 여럿일 수 있어서다(체외충격파가 단순/복잡 두 행). ' +
      '단일가면 minAmount 와 maxAmount 가 같으며, 이때는 범위로 표시하지 않는다.\n\n' +
